      ...
      return new Promise((ok, err) => {
        ...
        referenceIdToCallback.set(referenceId, (payload) => {
          ...
          if (messageType === "parseTsModule_deps") {
            const { isJsSrc, staticDependencies, dynamicDependencies } =
              messageData;
            ...
            if (messageType === "error") {
              ...
              contextMessage =
                "Unexpected parseTsModule() worker response at " +
                params.fullUrl;
            }
...
const withFreeWorker = (action) =>
  new Promise((ok, err) => {
    workerCallbackQueue.push((worker) =>
      Promise.resolve()
        .then(() => action(worker))
        .then(ok)
        .catch(err),
    ...
    const checkFree = () => {
      if (freeWorkers.size > 0 && workerCallbackQueue.length > 0) {
...
function tryFetchModuleSrcExtOrNot(url, jsx) {
  ...
  const urlOptions = [];
  const explicitExtension = EXPLICIT_EXTENSIONS.find((ext) =>
    url.endsWith("." + ext),
  );
  if (explicitExtension) {
    ...
    if (explicitExtension === "js") {
      ...
      return whenModule.catch(async (jsExtError) => {
        try {
          return await tryFetchModuleSrcWithExt(
            url.replace(/\.js$/, "") + ".ts",
          );
        } catch {
          throw jsExtError;
  ...
  return oneSuccess(urlOptions.map(tryFetchModuleSrcWithExt)).catch(
    async (tsExtError) => {
...
const WorkerManager = ({ compilerOptions }) => {
  ...
  const parseInWorker = async ({ url, fullUrl, tsCode }) => {
    ...
    const checksum = !crypto.subtle
      ? null
      : await crypto.subtle
          .digest("SHA-256", sourceCodeBytes)
          .then(bufferToHex);
    ...
    if (fromCache) {
      ...
      return withFreeWorker((worker) =>
        worker.parseTsModule({
          fullUrl,
          tsCode,
          compilerOptions,
        }),
  ...
  return {
    fetchModuleData: (url) =>
      fetchModuleSrc(url).then(({ fullUrl, tsCode }) =>
        parseInWorker({ url, fullUrl, tsCode }),
      ),
  };
};
