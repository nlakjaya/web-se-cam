    ...
    for (const dependency of fileData.staticDependencies) {
      ...
      if (!modulePromises[newUrl]) {
        ...
        if (jsCode.match(/(^|\s+)export\b/)) {
          ...
          const msg =
            "warning: circular dependency on " +
            baseUrl +
            " -> " +
            newUrl +
            ", variables will be empty in module top-level scope";
          console.warn(msg);
        }
        window[CACHE_LOADED][newUrl] = makeCircularRefProxy(
...
const LoadRootModule = async ({ rootModuleUrl, compilerOptions = {} }) => {
  ...
  const fetchDependencyFiles = async (entryUrl) => {
    ...
    while ((entries = Object.entries(urlToPromise)).length > 0) {
      if (--safeguard <= 0) {
        throw new Error(
          "Got into infinite loop while fetching dependencies of " + entryUrl,
        );
      }
