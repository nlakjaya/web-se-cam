...
const transformStatement = ({ statement, sourceFile, baseUrl, ts }) => {
  ...
  const consumeAst = (node) => {
    if (
      ...
      const url =
        ts.SyntaxKind[arg.kind] !== "StringLiteral"
          ? null
...
org.klesun.tsBrowser.ParseTsModule_sideEffects = ({
  ...
  const sourceFile = ts.createSourceFile(
    "ts-browser-generated-file." + extension,
    tsCode,
  ...
  for (const statement of sourceFile.statements) {
    ...
    if (kindName === "ImportDeclaration") {
      ...
      if (!importClause) {
        ...
        const assignedValue =
          "window[" +
          JSON.stringify(CACHE_LOADED) +
          "][" +
          JSON.stringify(depUrl) +
          "]";
        jsCodeImports +=
          es6ToDestr(tsCode, importClause) + " = " + assignedValue + ";\n";
      }
